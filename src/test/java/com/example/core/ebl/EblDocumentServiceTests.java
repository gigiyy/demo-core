...
class EblDocumentServiceTests {
	...
	public void setup() {
		...
		operations.insertAll(startupData()).as(StepVerifier::create).expectNextCount(3).verifyComplete();
	}

	@Test
	public void should_all_documents_when_findAll() {
		// given

		// when
		Flux<ElectronicBillOfLading> ebls = subject.findAll();

		// then
		StepVerifier.create(ebls)
			.expectNextCount(3)
